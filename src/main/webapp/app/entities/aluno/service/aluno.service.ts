...
type RestOf<T extends IAluno | NewAluno> = Omit<T, 'matriculaData'> & {
...
export class AlunoService {
  ...
  protected convertDateFromClient<T extends IAluno | NewAluno | PartialUpdateAluno>(aluno: T): RestOf<T> {
    return {
      ...aluno,
      matriculaData: aluno.matriculaData?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAluno: RestAluno): IAluno {
    return {
      ...restAluno,
      matriculaData: restAluno.matriculaData ? dayjs(restAluno.matriculaData) : undefined,
    };
  }

...
export class MateriaComponent implements OnInit {
  ...
  protected readonly materiaService = inject(MateriaService);
  ...
  trackId = (item: IMateria): number => this.materiaService.getMateriaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.materias.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IMateria[] | null): IMateria[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const materiasNew = this.materias();
      if (data) {
        for (const d of data) {
          if (materiasNew.some(op => op.id === d.id)) {
            materiasNew.push(d);
          }
        }
      }

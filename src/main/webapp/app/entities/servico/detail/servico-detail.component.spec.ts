...
describe('Servico Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./servico-detail.component').then(m => m.ServicoDetailComponent),
              resolve: { servico: () => of({ id: 7799 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load servico on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', ServicoDetailComponent);

      // THEN
      expect(instance.servico()).toEqual(expect.objectContaining({ id: 7799 }));
    });
  });

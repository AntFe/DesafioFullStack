...
export class ProfessorComponent implements OnInit {
  ...
  trackId = (item: IProfessor): number => this.professorService.getProfessorIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.professors.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IProfessor[] | null): IProfessor[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const professorsNew = this.professors();
      if (data) {
        for (const d of data) {
          if (professorsNew.some(op => op.id === d.id)) {
            professorsNew.push(d);
          }
        }
      }
      return professorsNew;
    }
    return data ?? [];
  }
